...
/**
 * Created by Nadine on 29.03.2018.
 */
public class PrintingImage extends Application {
    ...
    public PrintingImage(HeaderBitmapImage image) // перевизначений стандартнийконструктор
    {
        this.image = image;
    }
    @Override
    public void start(Stage primaryStage) throws Exception {
        ReadingImageFromFile.loadBitmapImage("C:\\Users\\Nadine\\Desktop\\2 семестр\\Комп.графика\\Lab3\\Lab3\\resources\\HAHA.bmp");
        ...
        for (int i=0;i<height;i++) // поки не кінець зображення по висоті
            for (int j=0;j<half;j++) // поки не кінець зображення по довжині
            {
                ...
                if(j*2<width) // так як 1 символ кодує 2 пікселі нам  необхідно пройти до середини ширини зображення
                {
                    ...
                    root.getChildren().add(cir); //додаємо об'єкт всцену
