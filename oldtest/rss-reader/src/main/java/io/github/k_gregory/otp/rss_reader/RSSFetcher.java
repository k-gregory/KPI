...
/**
 * Created by grego on 14.05.2017.
 */
...
public class RSSFetcher {
    ...
    public void fetch(){
        ...
        String[] words = s
                .split("[,.!?\"“” ]+|\\R+",0);
                */
        String[] words = extractTextFromHtml(data).toLowerCase().split("[,.!?\"“” ]+|\\R+",0);
        Map<String, Long> collect = Stream.of(words).map(w->{
        ...
        Stemmer s = new Stemmer();
        String ss = "continue";
        s.add(ss.toCharArray(), ss.length());
        s.stem();
        ss = s.toString();

        List<Map.Entry<String, Long>> l = new ArrayList<>(collect.entrySet());
        l.sort(Comparator.comparing(Map.Entry<String, Long>::getValue).reversed());
        ...
        rest.getForEntity("https://www.theguardian.com/world/rss", RSS.class)
                .addCallback(res->{
                    S
                    String desc =  res.getBody().channel.items.get(0).description;
