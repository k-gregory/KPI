...
public class Application {
    public static void main(String... args) throws IOException {
        ANTLRInputStream inputStream = new ANTLRInputStream("a=1-(2+2);b=3+4;a+(a+b)*2^t;");
        MatrixLexer matrixLexer = new MatrixLexer(inputStream);
        CommonTokenStream tokenStream = new CommonTokenStream(matrixLexer);
        MatrixParser parser = new MatrixParser(tokenStream);
        MatrixParser.DefProgContext prog = parser.defProg();
        MatrixBaseVisitor<Double> matrixBaseVisitor = new MatrixBaseVisitor<Double>() {
        ...
        System.out.println(matrixBaseVisitor.visit(prog));
    }
}
